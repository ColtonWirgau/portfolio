...
function EmergingSquid({ size, isInView, delay }: { size: number; isInView: boolean; delay: number }) {
  ...
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      {/* Body — starts as footer bg color, transitions to accent */}
      <motion.path
        d={SQUID_PATH}
        initial={{ fill: FOOTER_BG }}
        animate={isInView ? { fill: ['#2A2622', '#2A2622', '#2A2622', '#E8501C'] } : undefined}
        transition={{ duration: 4, delay, times: [0, 0.4, 0.7, 1], ease: 'easeInOut' }}
      />
      {/* Eyeball — starts as footer bg, fades to white */}
      <motion.ellipse
        cx="170.67"
        cy="473.09"
        rx="39.52"
        ry="42.83"
        initial={{ fill: FOOTER_BG }}
        animate={isInView ? { fill: ['#2A2622', '#2A2622', '#FFFFFF', '#FFFFFF'] } : undefined}
        transition={{ duration: 3, delay, times: [0, 0.3, 0.6, 1], ease: 'easeInOut' }}
      />
      {/* Pupil — starts as footer bg, fades to dark */}
      <motion.circle
        cx="170.67"
        cy="473.09"
        r="17.6"
        initial={{ fill: FOOTER_BG }}
        animate={isInView ? { fill: ['#2A2622', '#2A2622', '#111111', '#111111'] } : undefined}
        transition={{ duration: 3, delay, times: [0, 0.3, 0.6, 1], ease: 'easeInOut' }}
      />
    </svg>
  );
