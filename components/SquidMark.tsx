...
/**
 * The site's squid mark (same art as the header logo / work-section
 * watermark). Body fill is configurable; the eye can be turned off for
 * tiny sizes where it would read as noise.
 *
 * `direction="left"` bakes a -90° rotation into the SVG itself (viewBox
 * + group transform) rather than relying on a CSS `rotate`. That keeps
 * the element box equal to the horizontal footprint, so the mark isn't
 * clipped when an ancestor with `overflow: hidden` composites it during
 * an animation.
 */
export function SquidMark({
  ...
  return (
    <svg
      width={width}
      height={height}
      viewBox={left ? '0 0 852.51 341.34' : '0 0 341.34 852.51'}
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
      style={style}
    >
      {left ? <g transform="translate(0 341.34) rotate(-90)">{content}</g> : content}
    </svg>
  );
}
